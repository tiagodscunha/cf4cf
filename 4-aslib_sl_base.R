...
run_experiment_lr <- function(type,goal){
  ...
  if(type == "IR"){
    features <- as.data.frame(read.csv("metafeatures_landmarkers/B_IR.csv", sep=";"))
    performances <- read.csv("targets/IR.csv", sep=";")
  }
  else {
    features <- as.data.frame(read.csv("metafeatures_landmarkers/B_RP.csv", sep=";"))
...
save(IR,RP,file="results/meta_aslib_sl_base.Rda")