...
run_experiment_cf4cf_meta <- function(type, maxzeroes, iterations){
  ...
  for(t in 1:iterations){
    ...
    tmp <- c(
      cf4cf_meta_knn = cf4cf_meta_knn,
      cf4cf_meta_rt = cf4cf_meta_rt,
      cf4cf_meta_rfr = cf4cf_meta_rfr
    )
    names(tmp) <- c(paste0("cf4cf_meta_knn_",t),paste0("cf4cf_meta_rt_",t),paste0("cf4cf_meta_rfr_",t))
...
save(IR,RP,file="results/cf4cf_meta_mf_ndcg.Rda")
