...
normalizeMatrix <- function(df){
  df1 <- as.data.frame(BBmisc::normalize(as.matrix(df)))
...
predictionsToRankings <- function(pred){
  
  tmp <- lapply(unique(pred$dataset), function(dt){
    c(as.character(unique(pred$dataset)[dt]), 
      as.character(pred[which(pred$dataset == dt),]$algorithm))
  })
...
tuneLearner <- function(data,learner_name){
  
  if(learner_name == "regr.lm"){
    
    ps = makeParamSet(makeNumericLearnerParam("tol", lower = 0.01, upper = 1))
    
  }
  
  if(learner_name == "regr.xgboost"){
    
    ps = makeParamSet(makeNumericLearnerParam("eta", lower = 0.1, upper = 1),
                      makeNumericLearnerParam("gamma", lower = 0.1, upper = 1))
  }
  
  if(learner_name == "regr.svm"){
    
    ps = makeParamSet(
      makeDiscreteVectorParam("kernel", len=4, values = c("linear","polynomial","radial","sigmoid")),
      makeNumericLearnerParam("gamma", lower = 0.1, upper = 1)
    )
  }
  
  if(learner_name == "regr.ranger"){
    
    ps = makeParamSet(makeIntegerLearnerParam("num.trees", lower = 50, upper = 200))
  }
  
  
  if(learner_name == "regr.rpart"){
    
    ps = makeParamSet(makeIntegerLearnerParam("minsplit", lower = 2, upper = 15),
                      makeIntegerLearnerParam("maxdepth", lower = 2, upper = 10))
  }
  
  
  if(learner_name == "regr.rknn"){
    
    ps = makeParamSet(makeIntegerLearnerParam("k", lower = 2, upper = 10))
  }
  ...
  tuned =  tuneModel(data, regression, learner, design, metric = misclassificationPenalties)
