...
run_experiment_cf4cf <- function(type,maxzeroes,goal){
  ...
  if(type == "IR"){
    metafeatures <- read.csv("metafeatures_landmarkers/B_IR_final.csv", sep=";")  #landmarkers datasets have NAs
  }
  else{
    metafeatures <- read.csv("metafeatures_landmarkers/B_RP_final.csv", sep=";")
  }
...
save(IR,RP,file="results/alors_sl_base.Rda")
